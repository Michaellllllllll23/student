...
export default function ParentDashboard({ user, onLogout }: ParentDashboardProps) {
  ...
  const loadChildren = async () => {
    try {
      ...
      setChildren(data);
      if (data.length > 0) {
        setSelectedChild(data[0].students);
      }
    } catch (error) {
          ...
          <>
            <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
              ...
              <select
                ...
                onChange={(e) => {
                  const child = children.find((c) => c.students.id === e.target.value);
                  setSelectedChild(child?.students);
                }}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-lg font-bold text-purple-600">
                          {selectedChild.grade_level} - {selectedChild.section}
                        </p>
                      </div>
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{selectedChild.student_id}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Name</p>
                      <p className="font-semibold text-gray-900">
                        {selectedChild.first_name} {selectedChild.middle_name}{' '}
                        {selectedChild.last_name}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Email</p>
                      <p className="font-semibold text-gray-900">{selectedChild.email}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Contact</p>
                      <p className="font-semibold text-gray-900">
                        {selectedChild.contact_number}
                      </p>
                    </div>
                  </div>
                </div>
